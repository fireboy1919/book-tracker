/**
 * BookTracker Simple Student Invitation System
 * 
 * This version uses the existing teacher invitation data endpoint.
 * Teachers just copy the invitation key from the frontend and paste it below.
 * The key contains both the class ID and encryption key encoded together.
 * 
 * Setup Instructions:
 ...
 * 2. Copy the invitation key from your class page in the frontend
 * 3. Paste it as INVITATION_KEY below
 * 4. Use the functions in your spreadsheet
 */

// ========== CONFIGURATION ==========
// Copy this from the teacher dashboard (it contains both class ID and encryption key)
const INVITATION_KEY = ""; // Paste the compound key from the frontend
...
function validateConfig() {
  if (!INVITATION_KEY || INVITATION_KEY.trim() === '') {
    throw new Error('INVITATION_KEY not set. Please copy the invitation key from the teacher dashboard.');
  }
}
...
/**
 * Encrypts student invitation data using the class-specific key
 */
